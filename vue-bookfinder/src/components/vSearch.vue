    ...
    methods: {
        startSearch() {
            if (this.searchReq !== '') {
                ...
                this.isLoading = true;
                axios.get(`/api/request?search=${this.searchReq}`)
                    .then(response => {
                        const {time, result} = response.data;
                        console.log(`Search "${this.searchReq}" in ${time} sec`);
                        this.arrayOfArrays = result;
                        this.$emit('array-updated', result);
